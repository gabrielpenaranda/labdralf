...
class FacturaController extends Controller
{
  ...
  {
    $factura = Factura::where('nota_entrega_factura', '=', 'false')->orderBy('numero_factura', 'desc')->paginate(10);
    return view('dralf.factura.index')->with(['factura' => $factura])->with(['modulo' => $modulo]);
  }
  ...
  public function create($modulo)
  {
    ...
    if (count($lote) == 0)
    {
      session()->flash('warning', 'Debe incluir un lote antes de incluir una factura');
      return redirect()->route('facturas.index', ['modulo' => $modulo]);
    }
    else if (count($cliente) == 0)
    {
      session()->flash('warning', 'Debe incluir un cliente antes de incluir una factura');
      return redirect()->route('facturas.index', ['modulo' => $modulo]);
    }
    return view('dralf.factura.create')->with(['factura' => $factura])->with(['cliente' => $cliente])->with(['modulo' => $modulo]);
  }
  ...
  public function store(CreateFacturaRequest $request, $modulo)
  {
    ...
    $factura->saldo_factura = $request->get('monto_factura') + $request->get('iva_factura');
    $factura->nota_entrega_factura = false;
    $factura->iva_factura = $request->get('iva_factura');
  ...
  public function show(Factura $factura, $modulo)
  {
    ...
    return view('dralf.factura.show')->with(['factura' => $factura])->with(['detallefactura'=> $detallefactura])->with(['modulo' => $modulo]);
  }
  ...
  public function destroy(Factura $factura)
  {
    ...
    {
      session()->flash('error', 'No puede eliminar el factura, posee información relacionada');
      return redirect()->route('facturas.index');
    }
    else
    {
  ...
  public function detailIndex(Factura $factura)
  {
    ...
    return view('dralf.detalleFactura.index')->with(['factura' => $factura])->with(['detalle' => $detalle]);
  }
}
